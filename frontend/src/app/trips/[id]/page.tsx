...
export default function TripDetailsPage({
  ...
  const fetchTripDetails = async () => {
    try {
      ...
      if (tripData.tripToDestinations?.length) {
        const initialRadiusSettings: Record<string, number> = {};
        tripData.tripToDestinations.forEach((td) => {
          if (td.destination?.id) {
            initialRadiusSettings[td.destination.id] = 5000;
          }
        });
        setRadiusSettings(initialRadiusSettings);
      }
    } catch (error) {
  ...
  const fetchAllDestinations = async () => {
    try {
      const allDestinations = await destinationsApi.getAllDestinations();
      setAvailableDestinations(allDestinations);
    } catch (error) {
      console.error('Failed to load available destinations', error);
    }
  };
  ...
  if (!trip) {
    return (
      <div className="py-12 text-center">
        <h2 className="text-xl font-medium text-gray-600">Trip not found</h2>
